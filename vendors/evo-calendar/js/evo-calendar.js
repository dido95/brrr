(function (factory) {
    ...
    EvoCalendar.prototype.buildTheBones = function () {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup += '<div class="calendar-inner">' +
                ...
                '<tr><th colspan="7" data-value="ss"></th></tr>' +
    ...
    EvoCalendar.prototype.addEventIndicator = function (active_date, type,price,discount) {
        var _ = this, htmlToAppend, thisDate = _.$elements.innerEl.find('[data-date-val="' + active_date + '"]');
        if (thisDate.find('span.event-indicator').length === 0) {

            if(discount != null){
                thisDate.addClass('vs-calender-active vs-event-' + type).attr({'data-price-val':price,'data-discount':discount});
            }else{
        ...
        for (var i = 0; i < _.options.calendarEvents.length; i++) {
            for (var x = 0; x < _.monthLength; x++) {
                ...
                if (active_date == _.options.calendarEvents[i].date) {
                    _.addEventIndicator(active_date, _.options.calendarEvents[i].type, _.options.calendarEvents[i].price, _.options.calendarEvents[i].discount);
                }
                else if (_.options.calendarEvents[i].everyYear) {
